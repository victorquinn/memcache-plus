...
Connection.prototype.read = function(data) {
    ...
    if (data === 'ERROR' && this.queue.toArray().length > 0) {
        ...
        deferred.resolve(data);
        this.data = null;
    } else if (data.substr(0, 10) === 'STAT items') {
        // format is
        // STAT items:SLAB_ID:<key> <value>

        // pull out the SLAB_ID:<key> <value> portion
        var slabIdKey = data.substr(11).split(':');

        var slabId = slabIdKey[0].toString();

        // pullout <key> <value>
        var slabData = slabIdKey[1].split(' ');
        ...
        this.data.items[slabId][slabData[0]] = parseInt(slabData[1], 10);
    } else if (data !== '') {
        this.data = data;
