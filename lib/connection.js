...
function Connection(opts) {
    ...
    if (opts.onError) {
        ...
        this.onError = function onError(err) { console.error(err); };
    }
...
Connection.prototype.set = function(key, val, ttl) {
    ...
    //
    // Basically, it seems better to throw an error like this, pushing the handling
    // of this error to user space than it is to take a number, silently convert it
    // to a string, store it in memcache, then return a string later where an application
    // expected a number to be returned (as that's what it set).
    //
    // On the roadmap is to implement 'type safety' which means someone will be able to
    // set a value in memcache that is a string, number, object, etc. and actually get
    // that type of thing back from the get(), but until then we'll kick out a value
    // that is not a string because this seems much safer.
    assert(typeof val === 'string', 'Cannot set in memcache with a not string value');
